// src/controllers/organizerOperationsController.js
...
async function getCommittee(req, res, next) {
  try {
    ...
    // Merge registered users with default committee to ensure rich directory
    ...
    // Privacy control: If not authenticated or not organizer/admin, mask contact info
...
async function getTasks(req, res, next) {
  try {
    ...
    if (tasks.length === 0) {
      // Seed initial tasks for this event
      const initialTasks = [
        {
          eventId,
...
async function createTask(req, res, next) {
  try {
    ...
    logger.event(`Task created: "${task.title}" for event ${eventId}`);
...
async function updateTask(req, res, next) {
  try {
    ...
    logger.event(`Task updated: "${updatedTask.title}" -> ${updatedTask.status}`);
    socketEmitter.emitTaskUpdated(eventId, updatedTask);
...
async function deleteTask(req, res, next) {
  try {
    ...
    logger.event(`Task deleted: id=${taskId} for event ${eventId}`);
    socketEmitter.emitTaskDeleted(eventId, taskId);
...
async function batchCreateTasks(req, res, next) {
  try {
    const { id: eventId } = req.params;
...
async function sendMessage(req, res, next) {
  try {
    ...
    logger.socket(`Chat message posted in event ${eventId} by ${name}: "${chatDoc.message}"`);
    socketEmitter.emitCommandChatMessage(eventId, chatDoc);
