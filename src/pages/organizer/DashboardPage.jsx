...
import TrackSelectorTabs from '../../components/organizer/TrackSelectorTabs';
import MultiTrackScheduleView from '../../components/organizer/MultiTrackScheduleView';
import { Calendar, Users, Radio, ArrowRight, Activity, Sparkles, CheckCircle2, Layers } from 'lucide-react';
...
export const DashboardPage = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 glass-panel p-6 rounded-3xl border border-slate-800">
        <div>
          ...
          <p className="text-xs text-slate-400">
            {activeEvent?.location || 'Main Stage Auditorium'} • Multi-Track Live War Room
          </p>
        </div>
      ...
      {/* Track Filtering Selector Tabs */}
      <TrackSelectorTabs
        tracks={tracks}
        selectedTrackFilter={selectedTrackFilter}
        onSelectTrackFilter={setSelectedTrackFilter}
        trackDelays={trackDelays}
      />

      {/* Multi-Track Parallel Schedule Grid */}
      <MultiTrackScheduleView
        tracks={tracks}
        agenda={agenda}
        selectedTrackFilter={selectedTrackFilter}
        trackDelays={trackDelays}
        onJumpSegment={jumpToSegment}
        onAdjustTrackDelay={updateTrackDelay}
      />

      {/* Live Active Stage Metrics & AI Telemetry */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Active Stage Countdown Card */}
        <div className="lg:col-span-2 glass-panel p-6 rounded-3xl border border-slate-800">
          <div className="flex items-center justify-between mb-4 pb-3 border-b border-slate-800">
            <span className="text-xs font-bold text-slate-300 uppercase tracking-wider flex items-center gap-2">
              ...
              <span>ACTIVE STAGE: {activeItem?.trackName || 'TRACK A — MAIN STAGE'}</span>
            </span>
              ...
              <div className="space-y-2 text-xs text-slate-400 bg-slate-900/50 p-3 rounded-xl border border-slate-800">
                <div className="flex justify-between">
                  ...
                  <span className="font-mono text-emerald-400 font-bold">
                    {trackDelays[activeItem?.trackId || 'track_a'] > 0
                      ? `+${trackDelays[activeItem?.trackId || 'track_a']}m Delay`
                      : 'On Schedule'}
                  </span>
                </div>
                <div className="flex justify-between">
