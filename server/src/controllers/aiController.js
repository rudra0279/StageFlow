  ...
  try {
    ...
    if (eventId) {
      ...
      if (sessionId) {
        ...
        if (selectedSessionDoc && !track) {
          currentTrack = selectedSessionDoc.track || selectedSessionDoc.room || currentTrack;
        }
      }

      if (currentTrack) {
        trackSessions = allSessions.filter(s =>
          s.track === currentTrack || s.room === currentTrack || s.trackId === currentTrack
        );
      ...
      nextSessionDoc = (liveSession ? upcomingSessions.find(s => s.orderIndex > liveSession.orderIndex) : null) ||
        upcomingSessions[0] ||
        selectedSessionDoc ||
        trackSessions[0];
    }

    const activeSession = selectedSessionDoc || nextSessionDoc;
    const speakerDoc = activeSession?.speakerId;

    const speakerName = speakerDoc?.name || activeSession?.speakerName || 'the speaker';
    const speakerPronunciation = speakerDoc?.pronunciationGuide || 'Standard pronunciation';
    ...
    const sessionTitle = activeSession?.title || 'Upcoming Session';
    ...
    if (nextSessionDoc) {
      nextSessionObj = {
        ...
        speakerName: nextSessionDoc.speakerId?.name || nextSessionDoc.speakerName || 'TBA',
    ...
    otherTrackSessions.forEach(s => {
      ...
      if (!otherTracksMap[tName]) {
        otherTracksMap[tName] = {
          track: tName,
          title: s.title,
          speakerName: s.speakerId?.name || s.speakerName || 'TBA'
        };
      }
    });
    const otherTracks = Object.values(otherTracksMap);

    // Build custom track-isolated AI answers tailored to anchor actions
    ...
    // 4. Speaker introduction
      ...
      const spk = nextSessionObj?.speakerName || speakerName;
      const pron = nextSessionObj?.speakerPronunciation || speakerPronunciation;
